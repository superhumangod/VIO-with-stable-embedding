...
#ifndef SUKFLG_MSCKF_VIO_H
#define SUKFLG_MSCKF_VIO_H
...
namespace msckf_vio {
/*
 * @brief SukflgVio Implements the algorithm in
...
class SukflgVio {
  public:
    ...
    // Constructor
    SukflgVio(ros::NodeHandle& pnh);
    // Disable copy and assign constructor
    SukflgVio(const SukflgVio&) = delete;
    SukflgVio operator=(const SukflgVio&) = delete;
    ...
    typedef boost::shared_ptr<SukflgVio> Ptr;
    typedef boost::shared_ptr<const SukflgVio> ConstPtr;

    Eigen::MatrixXd infer_jacobian(const CAMState& cam_state,
                               const Eigen::Matrix<double,9,9>& P,
                               const Eigen::Matrix<double,9,9>& P_root,
...
typedef SukflgVio::Ptr SukflgVioPtr;
typedef SukflgVio::ConstPtr SukflgVioConstPtr;
